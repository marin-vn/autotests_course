...
def everything_for_your_cat(cats_data):
    """Оптимизирует хранение данных таким образом,
    ...
    :param: cats_data - списком кортежей, где каждый кортеж состоит из:
            Кличка котика, Возраст котика, Имя покупателя, Фамилия покупателя
    :return: new_str - строка, формат для одного покупателя:
            Имя_покупателя Фамилия_покупателя: Кличка_котика1, Возраст; ...; Кличка_котикаN, Возраст\n"""
    owner_cats_dict = {}
    for cat in cats_data:
        owner_name = cat[2] + ' ' + cat[3]
        if owner_name in owner_cats_dict:
            owner_cats_dict[owner_name].append(cat[:2])
        else:
            owner_cats_dict[owner_name] = [cat[:2]]
    # Организуем данные в строку по каждому владельцу
    organized_data = ''
    for owner, cats in owner_cats_dict.items():
        cat_str = ''
        for cat in cats:
            cat_str += f'{cat[0]}, {cat[1]}; '
        cat_str = cat_str[:-2]  # Убираем последнюю запятую и пробел
        organized_data += f'{owner}: {cat_str}\n'
