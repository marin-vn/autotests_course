...
def repeats(our_str):
    """Смотрит сколько раз каждый символ уже встречался
        :param: our_str - строка
        :return: new_str - строка, где числа показывают
        сколько раз каждый символ уже встречался
        (количество добавляется к символам с помощью постфикса формата _n)"""
    our_list = list(our_str)
    our_dict = {}
    new_str = ''
    for letter in our_list:
        our_dict[letter] = our_dict.get(letter, 0) + 1
        if letter in our_dict:
            new_str += f'{letter}_{our_dict[letter]}'
        else:
            new_str += f'{letter}_{1}'
    return new_str
