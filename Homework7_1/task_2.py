...
class PersonInfo:
    ...
    def new_salary(self):
        top3_letter = []
        for division in self.department:
            for letter in division:
                top3_letter.append(letter)
        our_dict = {}
        for letter in top3_letter:
            if letter in our_dict:
                our_dict[letter] += 1
            else:
                our_dict[letter] = 1
        sorted_letter = sorted(our_dict, key=lambda x: our_dict[x], reverse=True)[:3]
        total = sum(our_dict[letter] for letter in sorted_letter)
        return 1337 * self.age * total
