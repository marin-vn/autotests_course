...
try:
    ...
    sleep(3)
    search_line = driver.find_element(By.CSS_SELECTOR, '[data-qa="addressee-selector-root"] [data-qa="controls-Render__field"]')
    ...
    sleep(2)
    name = "О'Нил Шакил"
    fio = driver.find_element(By.CSS_SELECTOR, '.controls-Search__nativeField_caretFilled')
    fio.send_keys(name)
    ...
    sleep(3)
    mss = driver.find_elements(By.CSS_SELECTOR, '.msg-dialogs-item')
    assert len(mss) == 0, 'Удаленное сообщение отображается'
finally:
    driver.quit()
